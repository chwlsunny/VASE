...
import shutil
import data
from vocab_places import Vocabulary  # NOQA
from model import VSE
from evaluation_places import i2t, t2i, i2a, a2i, a2t, t2a, AverageMeter, LogCollector, encode_data
#from image_caption_dataset import ImageCaptionDataset, collate_fn
from image_caption_dataset import ImageCaptionDataset, collate_fn_two_streams, collate_fn_three_streams
...
def main():
    ...
    parser.add_argument('--data_path', default='/root/wenlong_workspace/code/VASE/',
    ...
    parser.add_argument('--vocab_path', default='/root/wenlong_workspace/code/VASE/vocab/',
    ...
    # Load data loaders
    if opt.num_streams==3:
        train_loader=torch.utils.data.DataLoader(ImageCaptionDataset(opt.data_train, opt.num_streams, vocab), batch_size=opt.batch_size, shuffle=True, num_workers=2, collate_fn=collate_fn, pin_memory=True, worker_init_fn=worker_init_fn)
        val_loader=torch.utils.data.DataLoader(ImageCaptionDataset(opt.data_val, opt.num_streams, vocab, image_conf={'center_crop':True}), batch_size=opt.batch_size, shuffle=False, num_workers=2, collate_fn=collate_fn, pin_memory=True)
    elif opt.num_streams==2:        
        train_loader=torch.utils.data.DataLoader(ImageCaptionDataset(opt.data_train, opt.num_streams, vocab), batch_size=opt.batch_size, shuffle=True, num_workers=2, collate_fn=collate_fn, pin_memory=True,worker_init_fn=worker_init_fn)
        val_loader=torch.utils.data.DataLoader(ImageCaptionDataset(opt.data_val, opt.num_streams, vocab, image_conf={'center_crop':True}), batch_size=opt.batch_size, shuffle=False, num_workers=2, collate_fn=collate_fn, pin_memory=True)
    else:
...
def validate(opt, val_loader, model):
    ...
    if opt.num_streams==2:
        # compute the encoding for all the validation images and speeches
        ...
        (r1i, r5i, r10i, medri, meanri) = a2i(
            img_embs, audio_embs, measure=opt.measure)
