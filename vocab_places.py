...
def from_places_json(path):
    with open(path, 'rb') as f:
        dataset=json.load(f)['data']
    captions=[]
    for i, d in enumerate(dataset):
        #captions.append(str(d['asr_text']))
        captions.append(d['asr_text'].encode('utf-8'))
        #captions.append(str(d['asr_text'].encode('utf-8')))
        #captions.append(d['asr_text'])
    return captions
...
def build_vocab(data_path, data_name, jsons, threshold):
    ...
    for path in jsons[data_name]:
        ...
        #full_path = os.path.join(os.path.join(data_path, 'data/'+data_name), path)
        if data_name=='places':
            #full_path = os.path.join(os.path.join(data_path, 'data/'+data_name), path)
            full_path = os.path.join(data_path, path)
        elif data_name in ['coco_precomp', 'coco', 'f8k_precomp', '10crop_precomp', 'f30k_precomp', 'f8k', 'f30k']:
            full_path=os.path.join(os.path.join(data_path, 'data/'+data_name), path)
        else:
            raise ValueError("The code can not process the %s dataset" %data_name)
        #pdb.set_trace()
        if data_name == 'coco':
            ...
            captions = from_places_json(full_path)
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='/home/chengwenlong/code/VASE/')
